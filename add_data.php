<?php
    ...
    if($gambar_mahasiswa !=""){
        ...
        if(in_array($jenis, $jenis_gambar) === true ){
            ...
            $query = "INSERT INTO mhs_db VALUES ('','$nim', '$nama', '$prodi', '$angkatan', '$nama_gambar_baru')";
            $result = mysqli_query($kon, $query);
            
            if(!$result){
                echo die("Query Error : ".mysqli_errorno($kon)."-".mysqli_errorno($kon));
            }else{
                echo "<script>alert('Data telah ditambahkan!');window.location='index.php';</script>";
            }
        }else{
            echo "<script>alert('hanya bisa file png,jpg dan jpeg!');window.location='tambah.php';</script>";
        }
    }else{
            $query = "INSERT INTO mhs_db (nim, nama, prodi, angkatan) VALUES ('$nim', '$nama', '$prodi', '$angkatan')";
